...
import { debounce } from '@/utils/bpm'
...
export default {
  ...
  methods: {
    ...
    handlerListChange(val){
      ...
      this.$store.commit('bpm/clearPCondition')
      ...
      const canUsedAsPCon = (conf, parent) => {
        ...
        const loop = (data, parent) => {
          ...
          if(Array.isArray(data)){
            ...
            ? vm.$store.commit("bpm/addPCondition", data) 
            : vm.$store.commit("bpm/delPCondition", data.formId)
          }
        }
        loop(this.drawingList)
        saveDrawingList(this.drawingList)
        this.$store.commit('bpm/updateFormItemList', this.drawingList)
   ...
   //  this.$router.push({ name: "jsxPreview", params: { formData: this.formData } }); 
    let routeData = this.$router.resolve({name: 'jsxPreview', query: { formData: this.formData }}); window.open(routeData.href, '_blank'); 
    },
    generate(data) {
    ...
    checkColItem (cmp) {
      if(!cmp) return false
      const isPcon = this.bpm.processConditions.find(t => t.formId == cmp.formId) ? true : false
    ...
    drawingItemDelete(index, parent) {
      ...
      this.$store.commit("bpm/delPCondition", parent[index].formId);
