  ...
  if (element.showLabel === false) {
    ...
    return (
      <el-col span={element.span} class={className}
        ...
        <el-form-item label-width={labelWidth ? `${labelWidth}px` : null}
          label={label} required={element.required}>
...
{element.tag==='el-tabs' ?<el-tabs type={element.type} tab-position={element.tabPosition} v-model={element.defaultValue}>{ element.options.map((item,index) => { 
  return  (<el-tab-pane label={item.label} name={item.value}  > 
   <draggable list={item.children} animation={0} group={group} disabled={isCustom} class="drag-wrapper">
            {element.defaultValue===item.value &&  child[index]  }
       
          </draggable></el-tab-pane>)
...
function renderChildren(h, element, index, parent) {
  ...
  if(element.tag==='el-tabs'){
       var panes= element.options.map(item=>{

       return   item.children.map((el, i) => {
