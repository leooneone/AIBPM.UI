  ...
  <div class="right-board">
    ...
    <div class="field-box">
      ...
      <el-scrollbar class="right-scrollbar">
           ...
           <el-form-item label="选项卡位置" v-if="activeData.tabPosition !== undefined">
            ...
            <div style="margin-left: 20px;">
              <el-button
                ...
                type="text"
                @click="activeData.options.push({label:'New Tab',value:(new Date()).getTime().toString(),children:[]});"
              >添加选项</el-button>
            </div>
            <el-divider />
          </template>
        ...
        <el-form-item v-if="activeData.showLabel !== undefined
            && activeData.labelWidth !== undefined" label="显示标签"
          >
          ...
          <template v-if="activeData.tag === 'fc-org-select'">
            ...
            <el-form-item label="类型" >
              <el-select v-model="activeData.tabList" size="small" multiple>
                <el-option label="dep" value="dep" />
    ...
    requireChange(required) {
      ...
      if (required && !this.activeData.multiple) {
        ...
        if( isRangeCmp && !showDuration){
          this.$store.commit("bpm/delPCondition", this.activeData.formId);
          return 
        }
        this.$store.commit("bpm/addPCondition", this.activeData);
      } else {
        this.$store.commit("bpm/delPCondition", this.activeData.formId);
      }
    },
