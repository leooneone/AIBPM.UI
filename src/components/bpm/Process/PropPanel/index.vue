...
<template>
  ...
  >
        ...
        <el-tab-pane :label="'设置' + (value.type === 'approver' ? '审批人' : '发起人')" name="config">
          ...
          <div v-else-if="value.type === 'approver'">
            ...
            <div style="border-bottom: 1px solid #e5e5e5;padding-bottom: 1rem;">
              ...
              <div  v-else-if="approverForm.assigneeType === 'form'"   class="option-box">
             
                  <p>使用表单中的联系人作为审批人</p>
                
                <el-select v-model="approverForm.filedName" size="mini">

                  <el-option v-for="(item, index) in approvers" :key="index" :label="item.label" :value="item.value"
            ...
            <div class="option-box" style="border-bottom: 1px solid #e5e5e5;" v-if="orgCollection[approverForm.assigneeType] && orgCollection[approverForm.assigneeType].length > 1 || ['optional','form'].includes(approverForm.assigneeType)">
              <p>多人审批时采用的审批方式</p>
              <el-radio v-model="approverForm.SignType"  label="string" class="radio-item">串签（逐个审批，须所有审批人同意）</el-radio>
              <br>
              <el-radio v-model="approverForm.SignType"  label="counterAnd" class="radio-item">会签（须所有审批人同意）</el-radio>
              <br>
              <el-radio  v-model="approverForm.SignType"  label="counterOr" class="radio-item">或签（一名审批人同意或拒绝即可）</el-radio>
            </div>
          </div>
    ...
    <section  v-if="value && isCopyNode()" style="padding-left: 1rem;">
      <p>抄送人</p>
      <fc-org-select ref="copy-org" v-model="properties.participants" buttonType="button" title="抄送人" />
      <br>
...
export default {
  ...
  data() {
    return {
      ...
      contactors:[],//从vuex中获取的可以作为流程图审批人的集合
  ...
  computed: {
    ...
    usedFormItems(){
      return this.bpm.formItemList
    },
    approvers(){

      return this.contactors.map(item=>{return {label:item.label,value:item.vModel}})
    }
  },
  directives: {
    Clickoutside
  ...
  methods: {
    ...
    approverNodeConfirm() {
      ...
      this.approverForm.participants = this.orgCollection[assigneeType]
      
      console.log('this.approverForm.participants') 
      console.log(this.approverForm.participants)
    ...
    initApproverNodeData() {
      ...
      this.contactors=JSON.parse(JSON.stringify(this.bpm.processConditions)).filter(item=>item.tag==='fc-org-select')
      ...
      const participants = this.approverForm.participants
      this.resetOrgColl()
      if (Array.isArray(this.approverForm.participants)) {
        this.orgCollection[this.approverForm.assigneeType] = participants
      }
    ...
    initConditionNodeData(){
      ...
      this.pconditions = JSON.parse(JSON.stringify(this.bpm.processConditions));
