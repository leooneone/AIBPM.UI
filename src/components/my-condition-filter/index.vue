<template>
  ...
  >
    <template #default="{ node, data }">
      ...
      <template v-else>
        <span>
          <el-select v-model="data.field" :loading="loading" style="width:200px;" filterable remote :remote-method="remoteMethod" @change="(value)=>onChangeField(value, data)">
            <el-option
...
export default {
  ...
  methods: {
    ...
    async getData(query) {
      ...
      window.leo=res
      this.fields =res.data.list.map(function(item) {
