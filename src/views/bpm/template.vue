...
import { add as addTpl,get } from '@/api/bpm/template'
...
export default {
  ...
  mounted() {
    get({id:this.templateId}).then(res => 
    { 
      if(res.data!==null&&res.data.config!==null)      
        this.mockData = JSON.parse(res.data.config); 
      else 
      this.mockData=this.defaultConfig
