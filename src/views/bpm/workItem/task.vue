<template>
  <my-container v-loading="pageLoading">
    ...
    <el-table-column
      ...
      <el-table-column
       ...
       <a :href="'/detail?instanceId='+row.instanceId" target="_blank">   {{row.referenceNo}}</a>
        </template>
      </el-table-column>
      ...
      <el-table-column
       ...
       <a :href="'http://localhost:9000/workitem?workItemId='+row.id" target="_blank">   {{row.title}}</a>
        </template>
      </el-table-column>
  ...
  <el-table-column
    ...
    <!--新增窗口-->
    <my-window
      v-if="checkPermission(['api:bpm:workItem:add'])"
      title="新增待办事项"
      embed
      drawer
    ...
    <!--编辑窗口-->
    <my-window
      v-if="checkPermission(['api:bpm:workItem:update'])"
      title="编辑待办事项"
      embed
      drawer
...
export default {
  ...
  methods: {
    ...
    async getTask() {
      ...
      const res = await workItem.getTask(params)
