...
export default {
  data () {
    return {
      ///流程ID用于提交表单关联对应流程
      workflowId:this.$route.query.workflowId,//this.$route.params.workflowId,
  ...
  methods: {
    submitForm () {//提交表单
      ...
      this.$refs[this.confGlobal.formRef].validate(valid => {
        ...
        instance.submit({templateId:this.workflowId,instanceId:this.instanceId,activityCode:this.activityCode,form:this.formModel}).then(res=>{
            console.log(res)
        });
    ...
    //远程获取表单配置
    getConfigByAjax() {
      instance.open({tplId:this.workflowId,instanceId:this.instanceId}).then(res => {
  ...
  render (h) {
    ...
    return  <div class="preview-container" style={'width:' + this.containerWidth + '%;'}>Activity:{this.activity.properties.title}{this.activity.nodeId}
